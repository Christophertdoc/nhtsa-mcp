"""Tests for get_vehicle_variables MCP tool."""

from __future__ import annotations

import asyncio

import httpx
import pytest
import respx

from app.config import Settings
from app.main import AppContext
from app.nhtsa_clients.api_nhtsa_client import APINHTSAClient
from app.nhtsa_clients.vpic_client import VPICClient
from app.security.cache import AsyncTTLCache
from app.security.rate_limiter import RateLimiter


def make_ctx(app_ctx: AppContext):
    class FakeRequestContext:
        lifespan_context = app_ctx

    class FakeContext:
        request_context = FakeRequestContext()

    return FakeContext()


VPIC_BASE = "https://vpic.nhtsa.dot.gov/api"
NHTSA_BASE = "https://api.nhtsa.gov"

SAMPLE_VAR_LIST = {
    "Results": [
        {
            "ID": 1,
            "Name": "Other Make",
            "GroupName": "Make / Model",
            "Description": "Other Make Name",
        },
        {
            "ID": 2,
            "Name": "Other Model",
            "GroupName": "Make / Model",
            "Description": "Other Model Name",
        },
    ]
}

SAMPLE_VAR_VALUES = {
    "Results": [
        {"Id": 1, "Name": "2-Door Coupe"},
        {"Id": 2, "Name": "4-Door Sedan"},
    ]
}


@pytest.fixture
def app_ctx():
    settings = Settings(rate_limit_enabled=False)
    semaphore = asyncio.Semaphore(20)
    vpic_http = httpx.AsyncClient(base_url=VPIC_BASE)
    nhtsa_http = httpx.AsyncClient(base_url=NHTSA_BASE)
    return AppContext(
        vpic_client=VPICClient(vpic_http, semaphore, settings),
        nhtsa_client=APINHTSAClient(nhtsa_http, semaphore, settings),
        caches={
            "vin": AsyncTTLCache(maxsize=10, ttl=60),
            "ratings": AsyncTTLCache(maxsize=10, ttl=60),
            "recalls": AsyncTTLCache(maxsize=10, ttl=60),
            "complaints": AsyncTTLCache(maxsize=10, ttl=60),
            "cssi": AsyncTTLCache(maxsize=10, ttl=60),
            "vpic_ref": AsyncTTLCache(maxsize=10, ttl=60),
            "vpic_query": AsyncTTLCache(maxsize=10, ttl=60),
        },
        rate_limiter=RateLimiter(enabled=False),
        settings=settings,
    )


class TestGetVehicleVariables:
    @respx.mock
    @pytest.mark.asyncio
    async def test_full_list(self, app_ctx):
        respx.get(f"{VPIC_BASE}/vehicles/GetVehicleVariableList").mock(
            return_value=httpx.Response(200, json=SAMPLE_VAR_LIST)
        )
        from app.mcp_tools.vpic_variables import get_vehicle_variables_tool

        ctx = make_ctx(app_ctx)
        result = await get_vehicle_variables_tool(ctx)
        assert result["summary"]["count"] == 2
        assert result["results"][0]["variable_name"] == "Other Make"

    @respx.mock
    @pytest.mark.asyncio
    async def test_variable_values(self, app_ctx):
        respx.get(f"{VPIC_BASE}/vehicles/GetVehicleVariableValuesList/Body Class").mock(
            return_value=httpx.Response(200, json=SAMPLE_VAR_VALUES)
        )
        from app.mcp_tools.vpic_variables import get_vehicle_variables_tool

        ctx = make_ctx(app_ctx)
        result = await get_vehicle_variables_tool(ctx, variable="Body Class")
        assert result["summary"]["count"] == 2
        assert result["results"][0]["variable_name"] == "2-Door Coupe"
