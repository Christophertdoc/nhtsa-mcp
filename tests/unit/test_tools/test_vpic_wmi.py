    ...
    "Results": [
        {
            "CommonName": "Ford",
            "MakeName": "FORD",
            "ManufacturerName": "FORD MOTOR COMPANY, LLC",
...
class TestDecodeWMI:
    ...
    async def test_success(self, app_ctx):
        respx.get(f"{VPIC_BASE}/vehicles/DecodeWMI/1FA").mock(
            return_value=httpx.Response(200, json=SAMPLE_WMI_RESPONSE)
        )
        from app.mcp_tools.vpic_wmi import decode_wmi_tool

        ctx = make_ctx(app_ctx)
        result = await decode_wmi_tool(ctx, wmi="1FA")
        assert result["summary"]["count"] == 1
        assert result["results"][0]["manufacturer_name"] == "FORD MOTOR COMPANY, LLC"

    @respx.mock
    @pytest.mark.asyncio
    async def test_cache_hit(self, app_ctx):
        route = respx.get(f"{VPIC_BASE}/vehicles/DecodeWMI/1FA").mock(
            return_value=httpx.Response(200, json=SAMPLE_WMI_RESPONSE)
        )
        from app.mcp_tools.vpic_wmi import decode_wmi_tool

        ctx = make_ctx(app_ctx)
        await decode_wmi_tool(ctx, wmi="1FA")
        result = await decode_wmi_tool(ctx, wmi="1FA")
        assert result["summary"]["cache_hit"] is True
        assert route.call_count == 1

    @pytest.mark.asyncio
    async def test_invalid_wmi_rejected(self, app_ctx):
        ...
        with pytest.raises(ValidationError):
            await decode_wmi_tool(ctx, wmi="X")
